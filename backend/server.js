...
const UserSchema = new mongoose.Schema({
  ...
  // npm i crypto (important to do this in backend folder cd backend)
  accessToken: {
    type: String,
...
app.post("/register", async (req, res) => {
  ...
  // npm install bcrypt (important to do this in backend folder cd backend)

  // -------------------------------------
  // const code = [1, 2, 4, 4]; [] should not be stored in db
  ...
  try {
    ...
    if (password.length < 8) { // add rules here
      res.status(400).json({
        success: false,
